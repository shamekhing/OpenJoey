...
//	のもの。やねうらおが、いくつかの修正をした。('03/06/16)
//

#include <stdio.h>
#include <fstream>		//	fstream.h → fstream
#include <iostream>		//	iostream.h → iostream

//	すまんが一応、namespaceでくくらせてもらう
...
/**
	...
	vector<BYTE> v; // ←これを食わせてみる
	MD5 md5;
	//	↑ MD5(&v[0],v.size()) md5;
	//	ってな感じでコンストラクタで渡せばupdate～finalizeしたことになる
	md5.update(&v[0],v.size());	//	←	updateでどんどん渡すことも可能
	md5.finalize();				//	←	finalizeするとそこで終了する。
								//		以降updateは出来ない
...
// methods to acquire finalized result
  ...
  char *            hex_digest ();  // digest as a 33-byte ascii-hex string
	//	↑この2つの関数で得た戻り値は、delete [] しなければメモリリークするので注意!
