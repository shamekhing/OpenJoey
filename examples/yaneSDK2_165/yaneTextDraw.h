...
public:
	...
	void	SetTextPtr(LPCSTR lpszText);
	...
	//////////////////////////////////////////////////
	//	現在のコンテクスト位置を返す
	LPCSTR	GetTextPtrNow(void) { return m_lpStr; }

protected:
	LPCSTR	m_lpStr;			//	現在のコンテクスト位置
	LPCSTR	m_lpTextAdr;		//	←ファイルをオープンしたら、SetTextPtrでそのメモリ位置を与えること
...
public:
	...
	//　縦書き設定
	void	SetVertical(bool bVertical = true) { m_bVertical = bVertical; }	//　縦書きに設定する
...
//	virtual void OnDrawText(const CTextDrawContext&,int x,int y,DWORD moji)=0;	//	現在のコンテキストに基づいて文字を描画する
	virtual void OnDrawText(const CTextDrawContext&,DWORD moji,LPRECT lpDrawPos)=0;	//	RECTの最終調整が行える
	...
	void	SetContext(CFont&font,const CTextDrawContext& context,LPCSTR lpStr);

private:
	//	CStringScannerに委譲＾＾；
	bool	IsToken(LPCSTR &lp,LPCSTR lp2);	//	トークンチェック
	//	一致後は、その分だけlpは前進する
	LRESULT SkipTo(LPCSTR &lp,LPCSTR lp2);	//	指定トークンまで読み飛ばし
	//	一致後は、その分だけlpは前進する。'\0'に出くわしたら非0が返る
	LRESULT SkipTo2(LPCSTR &lp,LPCSTR lp2,LPSTR lp3); //	指定トークンまで読み飛ばし
	//	一致後は、その分だけlpは前進する。'\0'に出くわしたら非0が返る
	//	lp3(bufは呼び出し側で用意すること)には、スキップするまでの文字列が入る
	LRESULT SkipSpace(LPCSTR &lp);	//	スペース、タブ、改行を切り詰める
	//	'\0'に出くわしたら非0が返る
	...
	vector<LPCSTR> m_vTagList;	//	検出した未知のタグリスト
...
//	virtual void OnDrawText(const CTextDrawContext&,int x,int y,DWORD moji);
	virtual void OnDrawText(const CTextDrawContext&,DWORD moji,LPRECT lpDrawPos);
