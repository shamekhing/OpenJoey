...
class CFastPlaneARGB4565 /* 16ビットRGB 565 + A3ビット == 24ビット独自ARGB */ {
public:
	BYTE GetR() const { return (_rgb & 0xf800) >> (11-3); }
	void SetR(BYTE r) { _rgb = (_rgb & ~0xf800) | (((WORD)(r&0xf8))<<(11-3)); }
	BYTE GetG() const { return (_rgb & 0x07e0) >> (6-3); }
...
//		return *(WORD*)(&_b) | (((DWORD)_r) << 16);
	...
	CFastPlaneRGB888&  dmul (const X &src,BYTE alpha){
		DWORD alpha8 = (DWORD)CFastPlaneBlendTable::abyMulTable[alpha + ((DWORD)src.GetA() << 8)] << 8;
		BYTE r = CFastPlaneBlendTable::abyMulTable[src.GetR() + alpha8] + CFastPlaneBlendTable::abyMulTable2[ GetR() + alpha8];
		BYTE g = CFastPlaneBlendTable::abyMulTable[src.GetG() + alpha8] + CFastPlaneBlendTable::abyMulTable2[ GetG() + alpha8];
		BYTE b = CFastPlaneBlendTable::abyMulTable[src.GetB() + alpha8] + CFastPlaneBlendTable::abyMulTable2[ GetB() + alpha8];
		SetRGB(r,g,b);
		return *this;
	}
	template <class X>
...
*/
		...
		//	こいつがゼロのときは、RGBを計算するに値しない
			...
			//	上のパラメータでブレンドする
			BYTE r = CFastPlaneBlendTable::abyMulTable[src.GetR() + nAlphaSrcD8] + CFastPlaneBlendTable::abyMulTable[ GetR() + nAlphaDstD8];
			BYTE g = CFastPlaneBlendTable::abyMulTable[src.GetG() + nAlphaSrcD8] + CFastPlaneBlendTable::abyMulTable[ GetG() + nAlphaDstD8];
			BYTE b = CFastPlaneBlendTable::abyMulTable[src.GetB() + nAlphaSrcD8] + CFastPlaneBlendTable::abyMulTable[ GetB() + nAlphaDstD8];
			SetRGB(r,g,b);
		}
		return *this;
	}
...
public:
	CFastPlaneCopyMulAlpha(BYTE alpha) : _alpha(alpha){};// { _alpha=alpha; }
	//	↑ここで指定するのは、減衰率
...
public:
	CFastPlaneCopyMulAlphaAB(BYTE srcalpha,BYTE dstalpha) {
		_srcalpha=srcalpha;
		_dstalpha=dstalpha;
	}
	//	↑ここで指定するのは、減衰率
...
public:\
	CFastPlaneCopyAddMulConst(BYTE alpha) : _alpha(alpha) {}
	template <class _DST,class _SRC>
	inline void operator () (_DST& dst,_SRC& src) const {
		_SRC tmp = src;
		tmp *= _alpha;
...
public:
	CFastPlaneCopyAddMulConstSrcColorKey(_TYPE rgb,BYTE alpha) : _alpha(alpha) { _colorkey = rgb; }
	template <class _DST,class _SRC>
	inline void operator () (_DST& dst,_SRC& src) const {
...
public:
	CFastPlaneCopySubMulConst(BYTE alpha) : _alpha(alpha) {}
	template <class _DST,class _SRC>
	inline void operator () (_DST& dst,_SRC& src) const {
		_SRC tmp = src;
		tmp *= _alpha;
...
public:
	CFastPlaneCopySubMulConstSrcColorKey(_TYPE rgb,BYTE alpha) : _alpha(alpha) { _colorkey = rgb; }
	template <class _DST,class _SRC>
	inline void operator () (_DST& dst,_SRC& src) const {
		if (src.GetRGB() != _colorkey.GetRGB()){
			_SRC tmp = src;
...
public:
	CFastPlaneBlendMulAlphaSrcColorKey(const _TYPE dw,const BYTE byRate): _colorkey(dw),_nRate(byRate){}
...
public:
	CFastPlaneBlendMulAlphaSrcColorKeyAB(_TYPE dw,BYTE bySrcRate,BYTE byDstRate): _colorkey(dw),_nSrcRate(bySrcRate),_nDstRate(byDstRate){}
...
private:
	_TYPE	_colorkey;
	BYTE	_nSrcRate;	//	転送元半透明比率
	BYTE	_nDstRate;	//	転送先半透明比率
};
...
public:
	CFastPlaneConstMul(BYTE nRate): _nRate(nRate) {}
...
public:
	...
	static LRESULT Effect(SrcClass _src,CSurfaceInfo* pInfo,EffectFunctor f,LPRECT lpRect=NULL){
	...
	static LRESULT EffectMosaic(SrcClass _src,CSurfaceInfo* pInfo,int nMosaicLevel,LPRECT lpRect=NULL){
		WARNING(pInfo->GetPtr() == NULL,"CFastPlaneEffect::EffectでpInfo->GetPtr() == NULL");
		RECT r = pInfo->GetClipRect(lpRect);
		LONG lPitch	 = pInfo->GetPitch();
		SrcClass* pSurface = (SrcClass*)pInfo->GetPtr();
	...
	static LRESULT Blt(SrcClass _src,CSurfaceInfo* lpSrcInfo,DstClass _dst
		,CSurfaceInfo* lpDstInfo,BltFunctor f,CSurfaceInfo::CBltInfo* pInfo){
...
//					lpDst++;			//	転送先のXを加算
	...
	static LRESULT EffectDouble(SrcClass _src,CSurfaceInfo* pInfo
...
//				p++;
	...
	static LRESULT BltDouble(
		SrcClass _src,CSurfaceInfo* lpSrcInfo,
...
//					lpDst++;			//	転送先のXを加算
	...
	static LRESULT Morph(
		...
		CSurfaceInfo*	lpSrcInfo,			// 転送元プレーンの情報
...
//		DstClass*	lpDstBefore; // でもこっちは使わなくてもOK
	...
	,CSurfaceInfo* lpDstInfo,BltFunctor f,int x,int y,LPRECT lpSrcRect=NULL,LPSIZE lpDstSize=NULL,LPRECT lpClipRect=NULL){
...
protected:
	...
	static LRESULT Clipping(CSurfaceInfo*lpDstInfo,CSurfaceInfo*lpSrcInfo,
