...
class CFastPlane : public ISurfaceDefault {
/**
...
//	void	SetDirectDrawSurface(bool bEnable);	//	DirectDrawSurfaceにするのか
	...
	*/
	...
	LRESULT		FadeEffect(BYTE nFadeRate,LPRECT lpRect=NULL){ return 0; } // 未実装(todo)

	///　α値を反転させる（αサーフェースに対してのみ有効）
	LRESULT		FlushAlpha(LPRECT lpRect=NULL) { return 0; } // 未実装(todo)
...
protected:
	...
	virtual LRESULT	InnerLoad(const string& szFileName);
...
//	static set<CFastPlane*>	m_lpPlaneList;
	...
	//	内部YGA画像の読み込み用
	LRESULT InnerLoadYGA(const string& szFilename);
