...
//	キーに対するデータの書き込み
void	CStringMap::Write(const string& szKey,const string& szData){
	map<string,string>::iterator it;
...
void	CStringMap::Write(const string& szKey,LONGLONG nData){
	string s;
...
//	キーに対するデータの追加書き込み
void	CStringMap::Append(const string& szKey,const string& szData){
	map<string,string>::iterator it;
	it = GetMap()->find(szKey);
	if (it==GetMap()->end()){
		//	そのkeyは存在しない
...
void	CStringMap::Append(const string& szKey,LONGLONG nData){
	string s;
	CStringScanner::NumToString(nData,s);
	Write(szKey,s);
}
...
//	キーに対するデータの読み出し
LRESULT	CStringMap::Read(const string& szKey,string& szData){
	map<string,string>::iterator it;
	it = GetMap()->find(szKey);
	if (it==GetMap()->end()){
		//	そのkeyは存在しない
...
LRESULT	CStringMap::Read(const string& szKey,LONGLONG& nData){
	string s;
...
string	CStringMap::GetStr(const string& szKey,const string& szDefault){
	string s;
	Read(szKey,s);
	if (s.empty()) {
...
int CStringMap::GetNum(const string& szKey,int nDefault){
	LONGLONG l;
	if (Read(szKey,l)!=0) return nDefault; // error時はnDefaultを返す
	return l;
}

LONGLONG	CStringMap::GetLongNum(const string& szKey,LONGLONG nDefault){
	LONGLONG l;
	if (Read(szKey,l)!=0) return nDefault; // error時はnDefaultを返す
	return l;
}
...
//	キーに対するデータの削除
bool	CStringMap::Delete(const string& szKey){
	//	削除したときにはtrue
...
LRESULT	CStringMap::ReadFromConfigFile(const string& szFileName){
	CFile file;
...
LRESULT CStringMap::WriteToConfigFile(const string& szFileName){
	CFile file;
	if (file.Open(szFileName.c_str(),"w")!=0) return 1;
