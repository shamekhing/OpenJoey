...
struct Killer
{
	...
	~Killer() {
		if(g_pCS!=NULL){
			CCriticalSection* p = g_pCS;
			g_pCS = NULL;
			delete p;
		}
	}
};
static struct Killer CSKiller;

void* operator new (size_t t){
	static BYTE aanHeap[g_nBlockNum][g_nBlockSize];	//	ヒープメモリ
	//	ローカルで確保するのは、終了時に自動的に解放されるように

	// クリティカルセクションオブジェクトの構築
	// 解体は殺し屋が行う
	static bool bRecursive = false;
	if (g_pCS==NULL){
		if (!bRecursive){
			bRecursive = true;
			g_pCS = new CCriticalSection;	// 再帰する。
			bRecursive = false;
		} else {
			// g_pCS==NULLで再帰してきたらmallocする。
...
void operator delete(void*p){
	...
	// クリティカルセクションオブジェクトが解体されていたらNULLになっている
	if(g_pCS!=NULL) g_pCS->Enter();
