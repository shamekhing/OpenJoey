...
void	CTextDrawContext::SetTextPtr(LPSTR lpszText){
...
LRESULT	CTextDrawBase::GetRect(RECT&rc){
	...
	CTextDrawContext& context = m_context_next;
	CFont f;
	SetContext(f,context,(LPSTR)&m_dwMojiLast);
...
//	ヘルパ関数 (CStringScannerに委譲)
	//	CStringScannerに委譲＾＾；
bool	CTextDrawBase::IsToken(LPSTR &lp,LPSTR lp2){
	return CStringScanner::IsToken(lp,lp2);
}
LRESULT	CTextDrawBase::SkipTo(LPSTR &lp,LPSTR lp2){
	return CStringScanner::SkipTo(lp,lp2);
}
LRESULT	CTextDrawBase::SkipTo2(LPSTR &lp,LPSTR lp2,LPSTR lp3){
	return CStringScanner::SkipTo2(lp,lp2,lp3);
}
LRESULT CTextDrawBase::SkipSpace(LPSTR &lp){
	return CStringScanner::SkipSpace(lp);
}
LRESULT	CTextDrawBase::GetStrNum(LPSTR &lp,int& nRate){
	return CStringScanner::GetStrNum(lp,nRate);
}
LRESULT	CTextDrawBase::GetNum(LPSTR &lp,int& nVal){
	return CStringScanner::GetNum(lp,nVal);
}
LRESULT CTextDrawBase::GetStrColor(LPSTR &lp,COLORREF& nFontColor){
	return CStringScanner::GetStrColor(lp,nFontColor);
}

////////////////////////////////////////////////////////////////////////

void	CTextDrawBase::SetContext(CFont&font,const CTextDrawContext& context,LPSTR lpStr){
	font.SetText(lpStr);
	if (!context.m_bBold){
...
////////////////////////////////////////////////////////////////////////
#ifdef USE_DIB32
void	CTextDrawDIB32::OnDrawText(const CTextDrawContext& context,int x,int y,DWORD dwMoji){
	CTextDIB32 t;
	SetContext(*t.GetFont(),context,(LPSTR)&dwMoji);
	t.UpdateText();
	m_dib.BltFast(&t,x,y);
}
void	CTextDrawDIB32A::OnDrawText(const CTextDrawContext& context,int x,int y,DWORD dwMoji){
	CTextDIB32 t;
	SetContext(*t.GetFont(),context,(LPSTR)&dwMoji);
	t.UpdateTextA();
	m_dib.BltFast(&t,x,y);
}
void	CTextDrawDIB32AA::OnDrawText(const CTextDrawContext& context,int x,int y,DWORD dwMoji){
	CTextDIB32 t;
	SetContext(*t.GetFont(),context,(LPSTR)&dwMoji);
	t.UpdateTextAA();
	m_dib.BltFast(&t,x,y);
...
void	CTextDrawPlane::OnDrawText(const CTextDrawContext& context,int x,int y,DWORD dwMoji){
	CTextPlane t;
	SetContext(*t.GetFont(),context,(LPSTR)&dwMoji);
	t.UpdateText();
	m_plane.BltFast(&t,x,y);
}
...
#ifdef USE_FastDraw
void	CTextDrawFastPlane::OnDrawText(const CTextDrawContext& context,int x,int y,DWORD dwMoji){
	CTextDIB32 t;
	SetContext(*t.GetFont(),context,(LPSTR)&dwMoji);
	t.UpdateText();
	m_plane.Blt((CDIB32*)&t,x,y);
		//	CDIB32 ⇔ CFastPlaneはサポートされている！
}
void	CTextDrawFastPlaneA::OnDrawText(const CTextDrawContext& context,int x,int y,DWORD dwMoji){
	CTextDIB32 t;
	SetContext(*t.GetFont(),context,(LPSTR)&dwMoji);
	t.UpdateTextA();
	m_plane.Blt((CDIB32*)&t,x,y);
...
void	CTextDrawFastPlaneAA::OnDrawText(const CTextDrawContext& context,int x,int y,DWORD dwMoji){
	CTextDIB32 t;
	SetContext(*t.GetFont(),context,(LPSTR)&dwMoji);
	t.UpdateTextAA();
	m_plane.Blt((CDIB32*)&t,x,y);
