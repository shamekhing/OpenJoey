...
public:
	virtual void* Create(const std::string& strClassName)=0;
	virtual LRESULT LoadPlugIn(const std::string& strFileName)=0;
	virtual LRESULT ReleasePlugIn(const std::string& strFileName)=0;

	//	DLL側がオブジェクトを登録するのに使う
	virtual void RegistClass(const std::string& strClassName,factory_base*)=0;
